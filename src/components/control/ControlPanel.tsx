import {AlignJustify, Camera, Pause, Play, Plus, SkipBack, Square, Trash2} from 'lucide-react'
...
import {clearKeyframesCommand, evenlySplitKeyframesCommand, upsertAndSelectCommand} from '@/store/commands'
...
export function ControlPanel({send}: ControlPanelProps) {
    ...
    const {activeShot, updateShotSettings, activeShotId} = useProjectStore()
    const shot = activeShot()
    const {keyframes} = useKeyframeStore()
    ...
    const handleEvenlySplit = () => {
        ...
        const updated = sorted.map((kf, i) => ({...kf, time: parseFloat((i * step).toFixed(3))}))
        execute(evenlySplitKeyframesCommand(updated))
        updated.forEach((kf) => send({type: 'SET_KEYFRAME', data: kf}))
    }
    ...
    const handleAddKeyframe = () => {
        ...
        execute(upsertAndSelectCommand(newKf))
    ...
    return (
        <div className="flex items-center gap-2 px-3 py-2 border-t border-[var(--color-border)] bg-[var(--color-surface)]">
            {/* Transport controls */}
            <Button size="icon" variant="ghost" disabled={disabled} onClick={() => send({type: 'SEEK', data: {time: 0}})}
                    title="Go to start">
                <SkipBack size={14}/>
            </Button>
            ...
            <div className="flex items-center gap-1 ml-1">
                ...
                <input
                    ...
                    onChange={(e) => {
                        ...
                        if (activeShotId) updateShotSettings(activeShotId, {durationSeconds: parseFloat(secs.toFixed(3))})
                    }}
            ...
            <Button size="sm" variant="ghost" onClick={() => execute(clearKeyframesCommand())} disabled={keyframes.length === 0}
            ...
            <Button size="sm" variant="ghost" onClick={handleEvenlySplit} disabled={keyframes.length < 2}
                    title="Distribute keyframes evenly across duration">
            ...
            {/* Capture keyframe */}
            <Button size="sm" variant="default" disabled={disabled} onClick={handleCaptureKeyframe}
                    title="Capture current camera pose as keyframe">
                <Camera size={13}/>
                Capture
            </Button>
        </div>

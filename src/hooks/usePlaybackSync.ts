...
export function usePlaybackSync() {
  ...
  const syncWallTime = useRef<number>(performance.now())
  ...
  useEffect(() => {
    const loop = (now: number) => {
      ...
      const activeShot = useShotStore.getState().activeShot()
      ...
      if (state === 'playing' && duration > 0) {
        ...
        const extrapolated = Math.min(syncGameTime.current + wallElapsed, duration)
        ...
        if (extrapolated > current || extrapolated < current - 0.5) {
